...
4. A simple daily sleep log with automatically calculated duration.
...
The app is built around five main views:
...
This is a one-record-per-day sleep journal. Each record belongs to the wake-up date and stores bedtime and wake-up time; the elapsed duration is calculated by the interface.
...
The sleep log is a compact notebook-style table with Date, Bedtime, Slept, and Wake-up columns. The editable row defaults to today, saves time changes automatically, and calculates the duration between bedtime and wake-up without storing redundant derived data.

Each wake-up date has at most one record. Saved rows are newest first and expose compact edit and delete icons; deletion requires confirmation. Times are displayed in 12-hour AM/PM format, while overnight calculations wrap across midnight.
...
Sleep records store a local wake-up date plus normalized bedtime and wake-up values. Duration is calculated in the interface and is not persisted.
...
### 14.2 Planning and Sorting Responsiveness
...
### 14.3 Fullscreen Responsiveness
...
## 16. Component-Level Design Summary

### 16.1 Top Control Components

- Sheet label
- Plan menu trigger
- Sleep log navigation button
- Sheet menu trigger
- Back navigation buttons
- History navigation buttons
- Save feedback indicator

### 16.2 Checklist Components
...
### 16.3 Planning Components

- Numbered task rows
- Text capture area
- Prioritize action button

### 16.4 Sorter Components
...
### 16.5 History Components

- Day group
- Completed/undone lists
- Entry edit controls

### 16.6 Sleep Log Components

- Editable date and time row
- Calculated duration cell
- Newest-first record rows
...
## 19. Suggested Future Design Direction
